...
HashTable.prototype.set = function(key, value) {
  ...
  if (this.count > .75 * this.SIZE) {
    ...
    for (let i = 0; i < copy.length; i++) {
      for (const prop in copy[i]) {
        this.set(prop, copy[i][prop]);
      }
    }
  }
  return this.count;
...
HashTable.prototype.remove = function(key) {
  let code = hashCode(key, this.SIZE);
  if (this.storage[code][key] !== undefined) {
    let result = this.storage[code][key];
    delete this.storage[code][key];
    this.count--;

    if (this.SIZE > 16 && this.count < Math.floor(.25 * this.SIZE)) {
