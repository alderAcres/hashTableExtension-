...
HashTable.prototype.set = function(key, value) {
  ...
  if (!this.storage[curHashKey]) {
    ...
    //add conditional to check if adding in would take the size to 75%. if so, recursive call to rehash? using helper function seems like a cleaner method

    //expanding case. it re
    if ((this.totalItems + 1) / this.SIZE >= .75) {
      console.log('inside expansion case');
      //double size of SIZE
      this.SIZE *= 2;

      //accessing collision objects
      for (let hashIndex of this.storage) {
        //accessing keys inside of collision objects
        for (let key in hashIndex) {
          const curVal = hashIndex[key];
          //is the below needed to delete old values after moving them?
          //also have to empty out collision object. should call remove.
          hashIndex[key] = undefined;
          this.set(key, curVal);
        }
      }
