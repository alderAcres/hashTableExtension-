...
HashTable.prototype.set = function (key, value) {
  ...
  if (this.num >= this.SIZE * 0.75) {
    this.SIZE *= 2;
    //unsure how to access every existing value and re-evlauate the hash code with the new size.
    //I'll need to access each value first, obvs
    //then to avoid overwriting as I go, maybe create a temporary storage array to later reassign to the storage array
    //I could use a while loop or a recursive function to iterate over the initial array reassign the hash code index and push to the new array
    //then reassign storage to the new, longer array
  }
  return this.num;
};
...
HashTable.prototype.remove = function(key) {
  ...
  if (this.SIZE > 16 && this.num < this.SIZE * 0.75) {
    this.SIZE /= 2
  }
