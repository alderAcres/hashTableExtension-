...
HashTable.prototype.set = function(key, value) {
  ...
  for(let el in this.storage){
    if(el !== undefined)filledCount++;
  }

  if((filledCount / this.SIZE) * 100 > 75){
    ...
    for(let el of tempArr){
      for(let keys in el){
        if(this.storage[hashCode(keys, this.SIZE)] === undefined){
          this.storage[hashCode(keys, this.SIZE)] = {};
          this.storage[hashCode(keys, this.SIZE)][keys] = value;
        }else{
          this.storage[hashCode(keys, this.SIZE)][keys] = value;
        }
      }
