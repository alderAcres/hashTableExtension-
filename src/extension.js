...
HashTable.prototype.set = function(key, value) {
  const potentialFillPercentage = (this.length + 1) / this.SIZE;
  if (potentialFillPercentage > 0.75) {
    this.SIZE *= 2;
    ...
    this.storage.forEach(el => {
      for (let key in el) {
        ...
        if (newStorage[newCode]) {
          newStorage[newCode][key] = tempVal;
        } else {
          let newTempObj = {}
          newTempObj[key] = tempVal;
          newStorage[newCode] = newTempObj;
        }
      }
...
HashTable.prototype.remove = function(key) {
  ...
  if (this.SIZE > 16) {
    const potentialFillPercentage = (this.length - 1) / this.SIZE;
    if (potentialFillPercentage < 0.25) {
      this.SIZE /= 2;
      const newStorage = new Array(this.SIZE)
      this.storage.forEach(el => {
        for (let key in el) {
  ...
  for (let a in this.storage[code]) {
    if (key === a) {
      ...
      if (JSON.stringify(this.storage[code]) === '{}') {
