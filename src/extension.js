...
HashTable.prototype.set = function(key, value) {
  ...
  // if counter is now > this.storage's length * 0.75
  if (counter > this.storage.length * 0.75) {
    // double this.storage length
    ...
    while (i < this.storage.length) {
      if (this.storage[i]) {
        // run set again
        // !COULDN'T COMPLETE BELOW! T-T
        let storedKeys = Object.keys(this.storage[i])
