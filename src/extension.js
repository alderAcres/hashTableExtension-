...
HashTable.prototype.set = function set(key, value) {
  // NOTE: if we're handling a collision, do not increase load
  // ASSUMPTION: load is only counting spaces in the array that have been taken up
  // not total items stored
...
HashTable.prototype.reset = function reset() {
  // if load/size > 0.75, then create a new HT that's double the size
  // for each item in current HT, call set on new HT to add the item
  // CONCERN/QUESTION: 
  // how do we get the key to rehash it? how to rehash? 
}
/**
...
HashTable.prototype.remove = function remove(key) {
  // NOTE: only decrease load if we need to empty out index in storage
  // get hKey
