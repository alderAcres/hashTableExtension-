...
HashTable.prototype.reHash = function (multiplier) {
  ...
  temp.forEach(({ key, value }) => newTable.set(key, value));
...
HashTable.prototype.set = function (key, value) {
  if (this.entries + 1 > this.SIZE * 0.75) this.reHash(2);
  let hash = hashCode(key, this.SIZE);
  if (!this.storage[hash]) {
    this.storage[hash] = new HashNode(key, value);
    this.entries++;
  } else {
    let current = this.storage[hash];
    if (current.key === key) {
      current.value = value;
      return this.entries;
    }
...
HashTable.prototype.remove = function (key) {
  if (this.entries - 1 < this.SIZE * 0.25) this.reHash(0.5);
  let hash = hashCode(key, this.SIZE);
  let temp = undefined;
  let current = this.storage[hash];
  let parent = current;
