...
HashTable.prototype.set = function (key, value) {
  const bucketPosition = hashCode(key, this.SIZE);
...
HashTable.prototype.get = function (key) {
  const bucketPosition = hashCode(key, this.SIZE);
  if (!this.storage[bucketPosition][key]) {
    return undefined;
  }
  return this.storage[bucketPosition][key];
};
...
HashTable.prototype.remove = function (key) {
  const bucketPosition = hashCode(key, this.SIZE);
  const removed = this.storage[bucketPosition][key];
  if (!this.storage[bucketPosition][key]) {
    return undefined;
  }
  delete this.storage[bucketPosition][key];
  this.storage[bucketPosition].length--;
