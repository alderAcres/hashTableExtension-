...
HashTable.prototype.remove = function (key) {
  let hashIndex = hashCode(key, this.SIZE);
  // if the key doesn't exist in storage at the specified hashindex, return undefined
  if (!this.storage[hashIndex].key) {
    return undefined;
  } else {
    let storage = this.storage[hasIndex].key;
    delete this.storage[hashIndex].key;
    return storage; //? Will this work? Possible not if value assoc with key is object...
  }
