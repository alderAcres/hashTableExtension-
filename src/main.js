...
HashTable.prototype.set = function (key, value) {
  ...
  // nothing at current. set this.storage[index] to the new HashNode
  if (current === undefined) {
    this.storage[index] = newHashNode;
    this.numberedStored++;
    return this.numberedStored;
  }

  // keep going down until you find the current.key === key.
  while (current != null) {
    if (current.key === key) {
      current.value = value;
      return this.numberedStored;
